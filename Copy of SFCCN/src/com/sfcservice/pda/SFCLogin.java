...
public class SFCLogin extends Activity implements OnClickListener,
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case MyConfig.ACCESSF:
				...
				MyTool.toastShow2(SFCLogin.this, "连接服务器失败");
				if (STATE == 0) {
					finish();
				}
				break;
	...
	public void btnClick() {
		// 基本判断+获取数据
		userCode = etUser.getText().toString().trim();
		password = etPassword.getText().toString().trim();

		if (!(userCode.equals("") || password.equals(""))) {
			getData();
		} else {
			MyTool.toastShow2(this, "用户名和密码不能为空");
			MyTool.playFailedSound();
		}
	...
	private void install() {
		File file = new File(getFilesDir(), "SFCCN.apk");
			...
			public void run() {
				try {
					...
					if (is != null) {
						...
						fileOutputStream = openFileOutput("SFCCN.apk",
								MODE_WORLD_READABLE);
