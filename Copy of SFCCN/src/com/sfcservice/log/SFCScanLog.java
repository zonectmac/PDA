 ...
 *
 */
public class SFCScanLog extends Activity implements OnScrollListener,
		OnClickListener, OnItemClickListener {
	/** Called when the activity is first created. */
	private ListView listView;
	private Button btnBack, btnSelect;
	private ArrayList<ScanBean> list20;
	...
	private void select(int STATUS) {
		this.STATUS = STATUS;
		list20 = new ArrayList<ScanBean>();
		if (!MyConnection.getMyConnection().getScanInfo20(list20, STATUS)) {
			tvNoLog.setVisibility(View.VISIBLE);
		}
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (firstVisibleItem + visibleItemCount == totalItemCount) {
			if (!MyConnection.getMyConnection().getScanInfo20(list20,
					STATUS)) {
	...
	private class MyAdaper extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			// TODO Auto-generated method stub
			if (convertView == null) {
				convertView = View.inflate(SFCScanLog.this,
						R.layout.sfc_log_item, null);
			}
			TextView tv1 = (TextView) convertView.findViewById(R.id.tv1);
			TextView tv2 = (TextView) convertView.findViewById(R.id.tv2);
			tv1.setText("客户ID: " + list20.get(position).getUserCode());
			tv2.setText("时间: " + list20.get(position).getStorageDate() + ", "
	...
	private void SFCPopWindow(int position) {
		if (p != null) {
			...
			tv2.setText(list20.get(position).getUserCode());
			tv3.setText(list20.get(position).getStatus()); 
			tv4.setText(list20.get(position).getStorageDate());
			tv5.setText(list20.get(position).getUploadDate());
		...
		v = LayoutInflater.from(this).inflate(R.layout.sfc_dialog_scan, null);
	...
	private void SFCPSelect() {
		...
		pSelect = new PopupWindow(vSelect, 300, LayoutParams.WRAP_CONTENT);
